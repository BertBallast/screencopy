use crate::dochters::globaal::{
    BEELD_OPTION, BESTAAND_BEELD, DASHED, DESKTOP_MAP, LAATSTE_FILE_PATH, LAATSTE_PIJL_PATH, LAATSTEBEELD, MAX_AANTAL_GELIJKE_FILENAMEN, OPTIES_ON, PIJL_TEKST_GROOT, PIJLENKOKER, WAYLAND
};
use crate::dochters::globaal::{
    BERICHT, BESTANDSNAAM, MAAK_UITSNEDE, MENU_POS, MONITOR_WISSEL, DOELSCHERM, KLEUR
};
use crate::dochters::optie_venster::wayland_rand;
...
impl MonitorFunctions for Monitor {
    fn xsize(&self) -> f32 {
...
/*
///Opvragen eigenschappen van de andere Monitor
...
//*
pub fn _findmouse() -> Pos2 {
    let mouse = Mouse::new();
    let p= mouse.get_position();
    println!("MP {:?}", p);
   //let u= PointerState::ui;
    //println!("Muis  {:?}", u(&ui));
    let q= p;
    let mut punt= Pos2{x: -1.0, y: -1.0};
    if q.is_ok() {
        let r= q.unwrap();
        punt = Pos2{x: r.x as f32, y: r.y as f32}
    }
    punt
}
// */
...
pub fn _printallmon() {
    let monitors = Monitor::all().unwrap(); // maak variabele met 'let ='
    for m in monitors {
        println!("MON {:?} {:?} {:?} {:?}", m.x(), m.y(), m.xpos(), m.ypos());
    }
...
pub fn bewaar_scherm_regio(rect: Rect, naam: String, uit_geheugen: bool) -> bool {
    // resultaat= succes
    ...
    //println!("MON {:?} {:?} {:?}", punt_1, punt_2, _findmouse());
    //_printallmon();
    match monitor_bevat_beide_punten(punt_1, punt_2) {
        Some(mon) => {
            ...
            nu zou met_beeld-rand kunnen worden opgeslagen; deze procedure kost veel tijd en is niet zo zinvol
            ?? is er een snellere manier om een gekleurde RgbaImage te maken, of een snellere overlay??
            */
            if capture_area.save(naam_string.as_str()).is_ok() {
...
pub fn contrastkleur(kleur: Color32, zwart_wit: bool) -> Color32 {
    let result: Color32;
    if zwart_wit {
        // black_white is the better option
        result = if (kleur.r() as i32)/2 + kleur.g() as i32 /*+ color.b() as i32*/ > 200 {
            // blue is a dark colour!,green makes other colors light
            Color32::BLACK
        } else {
            Color32::WHITE
        };
    ...
    result = Color32::from_rgba_unmultiplied(r, g, b, 255);
    result
}
...
pub fn cmd_line_argumenten() {
    wayland_rand();
    print!("OS: {}",std::env::consts::OS);
    let desktop= env::var("XDG_SESSION_DESKTOP");
    if desktop.is_ok() {print!("  {}", desktop.unwrap())};
    let system_type= env::var("XDG_SESSION_TYPE");
    if system_type.is_ok() {print!("  {}", system_type.clone().unwrap())};
    if system_type.is_ok() && system_type.clone().unwrap()=="wayland" {print!("  {}", system_type.clone().unwrap()); WAYLAND.set(true)};    
    println!();
    /* put an extra '/' before '/*'  to activate the commenrted code! */
    if os!= "windows" {
        let key = "XDG_SESSION_TYPE";
        let val= env::var(key);
        for (key, val)in std::env::vars() {
            println!("key:  {:20}   val:  {:20} ", key, val);
        }
        match val {
            Ok(v) => {
                println!("XDG {:?}", v);
...
/*
pub fn desktop_wayland() -> bool{
    let system_type= env::var("XDG_SESSION_TYPE");
    if system_type.is_ok() && system_type.unwrap()== "wayland" {
        return true;
    };
    false
}
