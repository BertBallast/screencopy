use eframe::egui::{
    self, Color32, Context, Key, Painter, Pos2, Rect, Stroke, StrokeKind, ViewportBuilder,
};
...
use crate::dochters::gereedschap::{hoofdscherm, Knop};
use crate::dochters::globaal::{
    BEELD_OPTION_TEXTURE, BERICHT, GESTART, Gestart, HANDLEIDING_GEBRUIK, HANDLEIDING_PIJLEN, HandleidingGebruik, MENU_POS, MENU_TYPE, MenuType, RECHTHOEK_OPSLAAN, SCHOONMAKEN, TEXT_INPUT
};
use crate::dochters::text_input::text_inp;
use crate::dochters::{
    gereedschap::{
        bewaar_scherm_regio, color_image_to_image_buffer, dichtbij, forceer_in_rect,
        image_buffer_to_color_image, load_image_from_path, maak_pijl_naam, naam_zonder_pad,
        pijltonen, plaats_in_rect, plaats_in_scherm, EigenTrait4LocalKey, PijlWijziging, Pijltje,
    },
    globaal::{
...
use super::gereedschap::MonitorFunctions;
...
pub fn pijlen_app() {
    ...
    let pijlenscherm = hoofdscherm();
    let position = Pos2 {
        //x: 0.0, y: 0.0
        x: pijlenscherm.xpos(),
        y: pijlenscherm.ypos(),
    };
    let windowsize = [pijlenscherm.xsize() - 1.0, pijlenscherm.ysize() - 1.0]; // -1.0 hier mogelijk niet nodig

    let mut options = eframe::NativeOptions {
        viewport: ViewportBuilder::default()
            .with_position(position)
            .with_decorations(false)
            //            .with_title_shown(false)      // overbodig en storend in linux
            //            .with_fullscreen(true)        // overbodig en storend in linux
            //            .with_titlebar_shown(false)   // overbodig en storend in linux
...
impl eframe::App for PijlenApp {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if SCHOONMAKEN.get() == false {
            ...
            bediening_functie(ctx.clone()); //werkt niet goed meer
...
pub fn pijlen_functie(zelf: &mut PijlenApp, ctx: Context) {
    ...
    match lees_opgeslagen_uitsnede(ctx.clone()) {
        ...
        Some(txh) => {
            // bewerk de opgeslagen afbeelding
            bewerk_afbeelding(
...
fn bewerk_afbeelding(
    ...
    egui::CentralPanel::default().show(&ctx.clone(), |ui| {
        ...
        // de afbeelding staat nu op het scherm, en kan met schuifjes (sliders) in het optie-venster worden verplaatst
        ...
        RECHTHOEK_OPSLAAN.set(beeldrect); // een gewijzigde beeldrect teruggekregen van 'teken_pijlen', en vastgelegd in RECHTHOEK_OPSLAAN
...
fn muis_acties_verwerken(ctx: Context, zelf: &mut PijlenApp, beeldrect: Rect, painter: Painter) {
    ...
    if SCHOONMAKEN.get() == false {
        if menu_rand {
            // MENU_POS= 4 betekent 'menu niet tonen'
    ...
    ctx.input(|k| {
        ...
        let muis = if let Some(ms) = k.pointer.hover_pos() {
            ...
            Pos2 {
                x: 30.0 + beeldrect.min.x,
                y: 30.0 + beeldrect.min.y,
            }
        };
        if k.pointer.primary_pressed() {
            // muisknop_Li indrukken wordt verwerkt
            if PIJL_EDIT.get() == true {
                PIJL_EDIT.set(false);
...
fn tijdelijke_markeringen(
    ...
    if SCHOONMAKEN.get() == false && PIJL_EDIT.get() || wijzig_punt.pijlnummer.is_some() {
        // markeer de pijlwijziging met een cirkel op de pijl, hetzij schacht, het zij punt of staart
        if wijzig_punt.schacht == true {
            // markeer de juiste pijl met een cirkel middenop de schacht
        ...
        if wijzig_punt.schacht == false {
            // markeer de muisplek op de tip of de rug (achrterzijde) van de pijl
            match muis {
                Some(pnt) => {
...
pub fn opslaan_juiste_rechthoek(rect: Rect, positie: Pos2) {
    ...
    bewaar_scherm_regio(rect_corr, bestandsnaam, false);
    OPSLAAN_VRAAG.set(false);
    OPSLAAN.set(false);
    SCHOONMAKEN.set(false);
}
